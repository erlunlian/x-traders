...
class XTweet(SQLModel, table=True):
    ...
    reply_to_tweet_id: Optional[str] = Field(default=None, sa_column=Column(String(100), nullable=True))
    conversation_id: Optional[str] = Field(default=None, sa_column=Column(String(100), nullable=True))
    in_reply_to_username: Optional[str] = Field(default=None, sa_column=Column(String(100), nullable=True))
    quoted_tweet_id: Optional[str] = Field(default=None, sa_column=Column(String(100), nullable=True))
    retweeted_tweet_id: Optional[str] = Field(default=None, sa_column=Column(String(100), nullable=True))
    
    # Entities stored as JSONB
    entities: Optional[Dict[str, Any]] = Field(default=None, sa_column=Column(JSONB, nullable=True))
    
    # Timestamps
    tweet_created_at: str = Field(sa_column=Column(String(100), nullable=False))
    fetched_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        sa_column=Column(DateTime(timezone=True), server_default=func.now()),
    )
