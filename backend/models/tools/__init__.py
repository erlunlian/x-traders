...
class BuyOrderInput(BaseModel):
    ...
    order_type: str = Field(default="MARKET", description="Order type: MARKET, LIMIT, or IOC")
    limit_price_in_cents: Optional[int] = Field(
        default=None, description="Maximum price in cents (required for LIMIT orders)"
    )
...
class SellOrderInput(BaseModel):
    ...
    order_type: str = Field(default="MARKET", description="Order type: MARKET, LIMIT, or IOC")
    limit_price_in_cents: Optional[int] = Field(
        default=None, description="Minimum price in cents (required for LIMIT orders)"
    )
...
class CreateTraderInput(BaseModel):
    ...
    initial_cash_in_cents: int = Field(
        default=100_000_000, description="Initial cash balance in cents (default $1,000,000)"
    )
